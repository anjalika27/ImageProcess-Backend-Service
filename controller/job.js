...
export async function submitJob(req, res) {
  ...
  try {
    const jobId = uuid();
    const data = await pool.query("insert into jobs values($1,$2,$3)", [
      jobId,
      "Ongoing",
...
export async function getJobStatus(req, res) {
  ...
  if (!jobId)
    return res.status(400).json({ error: "pass jobId as a query parameter" });
  try {
    const jobData = await pool.query('select * from jobs where "id"=($1)', [
      String(jobId),
    ]);
    if (jobData.rows.length === 0)
      return res.status(400).json({ error: `Can't find job with ID ${jobId}` });
    else {
